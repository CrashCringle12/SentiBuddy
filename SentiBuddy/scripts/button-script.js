document.addEventListener('DOMContentLoaded', () => {
    ...
    // queue onClick
    if (queueButton) {
        let isQueToggled = false;

        queueButton.addEventListener('click', () => {
            chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
                chrome.tabs.sendMessage(tabs[0].id, { type: 'toggle-queue-filtering' }, response => {
                    if (response && response.success) {
                        isQueToggled = response.enabled;
                        if (isQueToggled) {
                            queueButton.setText('Queue Filter: On');
                            queueButton.setColor('#107C10');
                        } else {
                            queueButton.setText('Queue Filter: Off');
                            queueButton.setColor('#D83B01');
                        }
                        console.log('Queue filtering toggle response:', response);
                    }
                });
            });
        });
        ...
        chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
            chrome.tabs.sendMessage(tabs[0].id, { type: 'get-queue-state' }, response => {
                if (response) {
                    isQueToggled = response.enabled;
                    if (isQueToggled) {
                        queueButton.setText('Queue Filter: On');
...
if (notificationButton) {
    ...
    // Initialize the button state based on stored settings
    chrome.storage.local.get({ desktopNotifications: false }, (items) => {
        isNotifToggled = items.desktopNotifications;
        if (isNotifToggled) {
            notificationButton.setText('Notifications: On');
            notificationButton.setColor('#107C10');
        } else {
        ...
        hashCheckButton.addEventListener('click', async () => {
            try {
                ...
                if (hashType != 'Unknown') {
                    ...
                    document.getElementById('results').textContent = 'No valid hash found in clipboard.';
                }
            } catch (error) {
                document.getElementById('results').textContent = 'Failed to access clipboard.';
            }
...
// Rest of your functions (defangURL, checkHashType, getScoreClass, toggleVisibility, updateProgress, displayResults) remain the same
...
async function defangURL() {
    try {
        ...
        await navigator.clipboard.writeText(defangedText);
        return 'Clipboard defanged!';
    } catch (error) {
